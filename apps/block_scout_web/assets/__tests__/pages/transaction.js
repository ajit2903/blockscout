...
describe('RECEIVED_NEW_TRANSACTION', () => {
  test('single transaction collated', () => {
    ...
    expect(output.transactionHashes).toEqual(['0x00'])
  })
...
describe('RECEIVED_NEW_TRANSACTION_BATCH', () => {
  ...
  test('batch with a mix of new and reorg (duplicate) transactions', () => {
    const state = Object.assign({}, initialState, {
      transactionCount: 5,
    ...
    const action = {
      ...
      msgs: [{
        transactionHash: '0x01',
        transactionHtml: 'replaced 1'
      }, {
        transactionHash: '0x02',
        transactionHtml: 'new 2'
      }]
    }
    const output = reducer(state, action)
  ...
  test('on page 2+ with a reorg (duplicate) transaction does not double count', () => {
    const state = Object.assign({}, initialState, {
      beyondPageOne: true,
      transactionCount: 5,
      transactionHashes: ['0x01']
