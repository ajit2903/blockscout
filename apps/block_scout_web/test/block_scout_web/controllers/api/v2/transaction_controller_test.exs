defmodule BlockScoutWeb.API.V2.TransactionControllerTest do
  ...
  describe "/transactions/{tx_hash}" do
    ...
    test "includes an empty authorization_list when the transaction has no signed authorizations", %{conn: conn} do
      tx =
        :transaction
        |> insert()
        |> with_block()
    ...
    test "includes authorization_list with signed authorizations sorted by index", %{conn: conn} do
      tx =
        :transaction
        |> insert()
        |> with_block()

      authorization_1 = insert(:signed_authorization, transaction_hash: tx.hash, index: 1)
      authorization_0 = insert(:signed_authorization, transaction_hash: tx.hash, index: 0)

      request = get(conn, "/api/v2/transactions/" <> to_string(tx.hash))

      assert response = json_response(request, 200)
      assert [item_0, item_1] = response["authorization_list"]

      assert item_0["nonce"] == authorization_0.nonce
      assert item_0["chain_id"] == authorization_0.chain_id
      assert item_0["address"] == to_string(authorization_0.address)

      assert item_1["nonce"] == authorization_1.nonce
      assert item_1["chain_id"] == authorization_1.chain_id
      assert item_1["address"] == to_string(authorization_1.address)
    end
