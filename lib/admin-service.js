...
function authenticatedAdmin(req, sessionSecret, githubAdminIds) {
  if (process.env.DISABLE_ADMIN_AUTH === 'true') {
    return {
  ...
  try {
    ...
    return {
      ...
      githubUserId: String(session.githubUserId),
...
function requireTargetAddressAccess(res, admin, body) {
  if (!body?.targetAddress || admin.canSetTargetAddress) return true;
...
async function dashboard(req, res) {
  ...
  const admin = requireAuthentication(req, res, settings);
  if (!admin) return;

  try {
    return json(res, 200, {
      ...(await getDashboard(settings.rpcUrl)),
      admin
    });
  } catch {
...
async function checkBalance(req, res) {
  ...
  if (!requireTargetAddressAccess(res, admin, body)) return;
